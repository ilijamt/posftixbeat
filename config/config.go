...
func mergeConfigFiles(configFiles []string, config *Config) error {
	...
	for _, file := range configFiles {
		...
		config.Postfixbeat.Prospectors = append(config.Postfixbeat.Prospectors, tmpConfig.Postfixbeat.Prospectors...)
	}
...
func (config *Config) FetchConfigs() {
	...
	configDir := config.Postfixbeat.ConfigDir
	...
	if len(config.Postfixbeat.Prospectors) == 0 {
